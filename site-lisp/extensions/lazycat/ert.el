...
;; Copyright (C) 2007 Christian M. Ohler
...
(defvar ert-find-test-regexp
  (concat "^\\s-*(deftest"
          ...
          "%s\\(\\s-\\|$\\)")
  "The regexp the `find-function' mechanisms use for locating test 
definitions.")
...
;;;###autoload
(defmacro* deftest (name () &body keys-and-body)
...
\(fn NAME () [:documentation DOCSTRING] [:expected-result TYPE] BODY...)"
  ...
  (destructuring-bind ((&key (expected-result nil expected-result-supplied-p)
    ...
    `(progn
       ...
       ;; This hack allows `symbol-file' to associate `deftest' forms
       ;; with files, and therefore enables `find-function' to work
       ;; with tests.  However, it leads to warnings in
       ;; `unload-feature', which doesn't know how to undefine tests
       ;; and has no mechanism for extension.
       (push '(deftest . ,name) current-load-list)
       ',name)))
...
Prompt with PROMPT.  By default, return DEFAULT-VALUE."
  (when (symbolp default-value) (setq default-value (symbol-name 
                                                     default-value)))
...
(defun ert-find-test-other-window (test-name)
  ...
  (find-function-do-it test-name 'deftest 'switch-to-buffer-other-window))
...
(defun ert-delete-test (test-name)
  ...
  (interactive (list (let ((default (thing-at-point 'symbol)))
                       (when default
                         ...
                         (when (or (string= default "nil") (intern-soft 
                                                            default))
...
a string -- Selects all tests that have a name that matches the string, a 
regexp.
     ...
     (etypecase universe
       ...
       (list (remove-if-not (lambda (test)
                              (and (ert-test-name test)
                                   (string-match selector (ert-test-name 
                                                           test))))
                            universe))))
...
(defun ert-run-test (test)
  "Runs TEST.  Returns the result and records it in TEST's corresponding slot."
...
(eval-and-compile
  (defun ert-special-operator-p (thing)
    ...
    (and (symbolp thing)
         (fboundp thing)
         (subrp (symbol-function thing))
         (eql (cdr (subr-arity (symbol-function thing))) 'unevalled)))
...
(defun ert-proper-list-p (x)
  "Returns non-nil if X is a proper list, nil otherwise."
  (loop
...
(defun ert-format-time-iso8601 (time)
  "Format TIME in the particular variant of ISO 8601 used for timestamps in 
ERT."
...
(deftest ert-test-should-failure-debugging ()
  (loop for (body expected-condition) in
        `((,(lambda () (let ((x nil)) (should x)))
          ...
          (,(lambda () (let ((x t) (y nil)) (should-not (ert-test-my-list x 
                                                                          y))))
           (ert-test-failed
...
(deftest ert-parse-keys-and-body ()
  ...
  (should (equal (ert-parse-keys-and-body '(:bar foo a (b))) '((:bar foo) (a 
                                                                           (b)))))
  (should (equal (ert-parse-keys-and-body '(:bar foo :a (b))) '((:bar foo :a 
                                                                      (b)) nil)))
  (should (equal (ert-parse-keys-and-body '(bar foo :a (b))) '(nil (bar foo :a 
                                                                        (b)))))
